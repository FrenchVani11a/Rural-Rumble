...
export default function LeaderboardPage() {
  ...
  const courseId = tab === "overall" ? undefined : tab;
  const { leaderboard, loading, players, scores } = useRealtimeLeaderboard(courseId);
  const { totalForPlayer } = useCanCounts();

  const leaderId = leaderboard.length > 0 && leaderboard[0].score ? leaderboard[0].player.id : null;
    ...
    <>
      ...
      <main className="px-4 py-6 md:py-10 max-w-5xl mx-auto">
        <div className="text-center mb-4">
          <h1 className="text-2xl md:text-4xl mb-1" style={{ fontFamily: "var(--font-bungee)" }}>
        ...
        {loading ? (
          ...
          <>
            ...
            {tab !== "overall" && (
              <>
                {/* Match play for Waverley + Whanganui */}
                {tab !== "rangatira" && (
                  <MatchPlayBoard players={players} scores={scores} courseId={tab} />
                )}

                {/* Race track */}
                <div className="mt-6">
                  {replayMode ? (
                    <RaceReplay players={players} scores={scores} onExit={() => setReplayMode(false)} />
                  ) : (
                    <>
                      {hasScores && (
                        <HoleSelector players={players} scores={scores} selectedHole={selectedHole} onHoleChange={setSelectedHole} />
                      )}
                      ...
                      <table className="w-full min-w-[500px]">
                        ...
                        <tbody>
                          {leaderboard.map((entry) => {
                            const coursePar = activeCourse?.holes === 9 ? 36 : 71;
            ...
            {/* Punishment Tracker — shown on all tabs */}
